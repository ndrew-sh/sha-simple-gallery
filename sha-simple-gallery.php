...
class SHA_Simple_Gallery {
	...
	private function init_admin_hooks() {
		...
		add_action( "manage_edit-{$this->_taxonomy}_columns", array( $this, 'override_gallery_grid_columns' ), 10, 2 );
		add_action( "manage_{$this->_taxonomy}_custom_column", array( $this, 'add_shortcode_value_to_admin_grid' ), 10, 3 );
		add_action( "{$this->_taxonomy}_add_form_fields", array( $this, 'add_extra_fields_to_add_form' ) );
		add_action( "{$this->_taxonomy}_edit_form_fields", array( $this, 'add_shortcode_field_to_edit_form' ) );
		add_action( "edited_{$this->_taxonomy}", array( $this, 'gallery_save_controller' ) );
		add_action( "create_{$this->_taxonomy}", array( $this, 'gallery_save_controller' ) );
    ...
    public function enqueue_styles_and_scripts() {
        ...
        if ( $this->_settings['load_css'] ) {
            wp_enqueue_style(
                $this->_plugin_slug,
                plugin_dir_url( $this->_plugin_file ) . 'frontend/css/styles.css',
                array(),
                $timestamp,
                'all'
            );
        ...
        if ( $this->_settings['load_js'] ) {
            wp_enqueue_script(
                $this->_plugin_slug,
                plugin_dir_url( $this->_plugin_file ) . 'frontend/js/scripts.js',
                array( 'jquery' ),
                $timestamp,
                false
            );
    ...
    public function render_guttenberg_block( $attributes ) {
        ...
        return do_shortcode('[cptgallery id="' . (int)$attributes['selectedGallery'] . '"]');        
    }
